function [x,j] = noisy_learned_simulator_step(x,i,new_S,neighbors,deg,d,dt,delta,noise)
%TEMP function: SAME as learned_simulator step, but with noise parameter
...
x = x + b(:,j)*dt + sigma(:,:,j)*sqrt(noise)*eta;
