...
%	TODO: better way to do this? Loading up structs if saved
	...
	for n = 1:N
        ...
        %TESTING: moving the subtraction of center to later in the
        %algorithm
        %local_L = local_L - local_L(:,1); %centering landmarks around center
        %local_X = X(:,:,n) - local_L(:,1); %likewise with X
        local_X = X(:,:,n);
...
%TODO: comments
	...
	for i = 1:num_nbr
		...
		if net_nbr(i) < n
            ...
            %if L_jn ~= L_nj
             %   fprintf("landmarks projections not equal! Should be! \n");
            %end
            sort(L_nj + embed_L(n).center)
            sort(L_jn + embed_L(j).center)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
