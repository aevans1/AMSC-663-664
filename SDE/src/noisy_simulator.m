function [path_end,regions_visited,X] = noisy_simulator(Xzero,m,T,f,dt,noise,regions,dist,plot_sim,plot_points)
%Simulator for linear SDE, using Euler-Maruyama method, with induced noise
%SDE is of form dx = f(x) dt + sqrt(noise)*dW, X(0) = Xzero
...
%Inputs:
	...
	%dt: timestep 
    %noise: parameter to make noisier steps in SDE, more exploration!
...
%Outputs:	
    ...
    %added noise line for 'noisy' simulator%%%%%%%%%%%%%%%%%%%%
    sq_noise = sqrt(noise);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	for i = 1:m
		...
		for j = 1:N
			...
			Xtemp = Xtemp + f(Xtemp)*dt + sq_noise*dW; %Take Euler-Maryuama step
			X(:,j+1) = Xtemp;
        end
